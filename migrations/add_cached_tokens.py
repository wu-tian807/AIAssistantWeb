...
def upgrade_usage_table():
    """添加cached_input_tokens和cached_input_cost列到usage表"""
    with app.app_context():
        # 检查列是否已存在
        inspector = db.inspect(db.engine)
        columns = [col['name'] for col in inspector.get_columns('usage')]
        needed_columns = ['cached_input_tokens', 'cached_input_cost']
        ...
        if missing_columns:
            ...
            create_table_sql = text("""
                CREATE TABLE usage_new (
                    ...
                    cached_input_cost FLOAT DEFAULT 0.0,
            ...
            copy_data_sql = text("""
                INSERT INTO usage_new (
                    ...
                    cached_input_tokens, cached_input_cost, input_cost, output_cost,
                    total_cost, created_at
                )
                SELECT 
                    id, user_id, model_name, tokens_in, tokens_out,
                    0, 0.0, input_cost, output_cost,
                    total_cost, created_at
                FROM usage
            """)
